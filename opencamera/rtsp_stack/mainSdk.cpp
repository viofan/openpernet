...
void supportMoreVideoAccess(RTSPServer *rtsp_server, int width, int height, char *type)
{
    ...
    if (strcmp(type, "store") != 0)
	{
		...
		videoSubsession->SetVideoParameters(width, height, video_bitrate, framerate, keyinterval, type);
	...
	getDeviceInfo(local_name, &local_port, local_mac_addr);
...
void realEntryMain() {
	...
	supportMoreVideoAccess(rtspServer, 352, 288, (char *)"live");
...
void play() {
	...
	//videoSource = (FramedSource*)EncoderVideoSource::createNew(*env, 352, 288, 10, 256, 5);
