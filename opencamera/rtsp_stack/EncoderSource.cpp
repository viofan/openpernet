...
#include "x264.h"
#ifdef __WIN32__
#define VIDEO_FILE		"e:\\media\\tempete.cif"
#define AUDIO_FILE		"e:\\media\\qlx.pcm"
#else
...
EncoderVideoSource::EncoderVideoSource(UsageEnvironment& env, unsigned int width, unsigned int height, unsigned int framerate,
	...
	fCodec(codec), fConfigBytes(NULL), fStartTime(0), x264_handle(NULL), fEncoderHandle(NULL)
{
	memset(mediaType, 0, sizeof(mediaType));
	memcpy(mediaType, type, strlen(type));
	if(strcmp(type, "live") == 0 || strcmp(type, "livehd") == 0)
		fp = fopen(VIDEO_CIF_FILE, "rb");
	if(strcmp(type, "mobile") == 0)
		fp = fopen(VIDEO_QCIF_FILE, "rb");
...
void EncoderVideoSource::MPEG4_doGetNextFrame()
{
	...
	else
	{
		...
		{  
			...
			videoGetFrameInfo(fp, mediaType, fBuffer, &size, &videoType);
		}
	}
	if(size <= 0)
		return ;
...
int EncoderVideoSource::getLivehdFrame(void)
{
	...
	if (strcmp(mediaType, "store") == 0)
	{
		getFileVideoFrame( fp, 0, (unsigned char *)tmpBuf, &size, &videoType, false);
	}
	else 
	{
		videoGetFrameInfo(fp, mediaType, tmpBuf, &size, &videoType);
	}
