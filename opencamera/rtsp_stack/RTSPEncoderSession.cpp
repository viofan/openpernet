...
::handleCmd_Unauthorized(void){
	...
	//delete this;
	writekey(key); // set zero of key
	getDeviceInfo(deviceIp, &devicePort, deviceMac);
	while(1)
	{
...
::handleCmd_DESCRIBE(char const* cseq, char const* urlSuffix,
	...
	if( strcmp(fPath, "store") == 0 )
	{
		...
		videoSubsession->SetVideoParameters(width, height, video_bitrate, framerate, keyinterval, fPath);
