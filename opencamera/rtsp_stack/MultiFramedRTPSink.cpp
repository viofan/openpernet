...
#if 0
	Debug(ckite_log_message, "frameSize = %d\n", frameSize);
	Debug(ckite_log_message, "numTruncatedBytes = %d\n", numTruncatedBytes);
	Debug(ckite_log_message, "[wayde]MultiFramedRTPSink::sendPacketIfNecessary0 packetSize = %d\n",fOutBuf->curPacketSize());
	Debug(ckite_log_message, "fIsFirstPacket = %d\n", fIsFirstPacket);
#endif
  if (fIsFirstPacket) {
