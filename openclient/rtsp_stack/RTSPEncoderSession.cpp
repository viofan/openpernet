...
: RTSPClientSession(ourServer, sessionId, -1, addr),
	...
	videofp(NULL){	
		...
		noteLiveness();
		Debug(ckite_log_message, "Encoder %x is created.\n", (int)this);
}

RTSPServer::RTSPEncoderSession::~RTSPEncoderSession() {
	Debug(ckite_log_message, "Encoder %x is destroyed.\n", (int)this);
}

void RTSPServer::RTSPEncoderSession::noteLiveness() {
	Debug(ckite_log_message, "RTSPServer::RTSPEncoderSession::noteLiveness\n");
	Debug(ckite_log_message, "fOurServer.fReclamationTestSeconds = %u\n", fOurServer.fReclamationTestSeconds);
	Debug(ckite_log_message, "fOurServer.fReclamationTestSeconds*1000000 = %u\n", fOurServer.fReclamationTestSeconds*1000000);
	Debug(ckite_log_message, "fLivenessCheckTask = %d\n",fLivenessCheckTask );
	Debug(ckite_log_message, "this = %p\n", this);
...
::livenessTimeoutTask(RTSPEncoderSession* encoder) {
	///////////////////////////////////////////////////////////
	Debug(ckite_log_message, "livenessTimeoutTask\n");
...
void RTSPServer::RTSPEncoderSession::handleRequestBytes(int newBytesRead) {
    ...
    Debug(ckite_log_message, "xxxxxxxxxxxxxxxxxxxxxxxxxxnoteliveness, %d bytes\n", newBytesRead);
...
#ifdef DEBUG
	ptr[newBytesRead] = '\0';
	fprintf(stderr, "RTSPEncoderSession[%p]::handleRequestBytes() read %d new bytes:%s\n", this, newBytesRead, ptr);
#endif
	...
	while (1)
	{
		...
		Debug(ckite_log_message, "Get rtsp message %d bytes.\n", msg_len);
		...
		if (strncmp((const char*)fRequestMsg, "RTSP/1.0", 8) == 0)
		{
			...
			{
				Debug(ckite_log_message, "parseRTSPResponseString() failed!\n");
				handleCmd_bad("1");
			}
			else
			{
				if (strcmp(resultCode, "200") == 0)
				{
					...
					if (strlen(challenge) > 0)	// Receive challenge from streaming server
					{
						...
						Debug(ckite_log_message, "challenge = %s\n", challenge);
						...
						int ret_len = crypto_encode(key, (unsigned char*)challenge, strlen(challenge),
									  (unsigned char*)challenge_resp, sizeof(challenge_resp));
						Debug(ckite_log_message, "challenge_resq = %s, len = %d\n", challenge_resp, ret_len);
						//fprintf(stderr, "challenge_resq = %s, len = %d\n", challenge_resp, ret_len);
					...
					Debug(ckite_log_message, "see other \n");
		...
		else
		{
			if (!parseRTSPRequestString((char*)fRequestMsg, msg_len,
				...
				if (strcmp(cmdName, "OPTIONS") == 0) {
				    ...
				    Debug(ckite_log_message, "cmdName %s not recognized.\n", cmdName);
...
::handleCmd_Unauthorized(void){
	...
	while(1)
	{
		Debug(ckite_log_message, "[Unauthorized] deviceMac = %s\n", deviceMac);
		//httpRequestKey(recvBuffer, deviceMac);
		Debug(ckite_log_message, "recvBuffer again = %s\n", recvBuffer);
		...
		{
			Debug(ckite_log_message, "[unauthorized] sleep\n");
			setSessionState(fPath, KEYFAILED);
			Sleep(30*60);// request again after half hour
		}
	}
	delete [] recvBuffer;
...
::handleCmd_SeeOtherServer(char const* fullRequestStr){
	...
	Debug(ckite_log_message, "host_name = %s, port = %d", host_name, atoi(port));
	sa.sin_family = AF_INET;
	sa.sin_addr.s_addr = inet_addr(host_name);
	sa.sin_port = htons(atoi(port));
	writeActiveServerInfo(host_name,(unsigned short)atoi(port));
...
::handleCmd_DESCRIBE(char const* cseq, char const* urlSuffix,
	...
	if (strlen(videoFile) > 10)
	{
		...
		Debug(ckite_log_message, "open new video file.\n");
		Debug(ckite_log_message, "DESCRIBE fNumStreamStates = %d\n", fNumStreamStates);
...
::handleCmd_SETUP(char const* cseq,
	...
	if (fSetupNum == 0)
	{
		...
		for (unsigned i = 0; i < fNumStreamStates; ++i) {
			...
			if (strcmp(fPath, "store") == 0)
			{
				Debug(ckite_log_message, "SETUP fStreamStates[%d].subsession = %x\n", i, fStreamStates[i].subsession);
			}
			fStreamStates[i].streamToken = NULL; // for now; reset by SETUP later
		}
	}
	Debug(ckite_log_message, "SETUP 1\n");
	...
	if (trackId != NULL && trackId[0] != '\0') { // normal case
		...
		Debug(ckite_log_message, "streamNum = %d\n", streamNum);
...
::handleCmd_withinSession(char const* cmdName,
	...
	if (fSession == NULL) { // There wasn't a previous SETUP!
	    Debug(ckite_log_message, "The fSession is NULL!\n");
		handleCmd_notSupported(cseq);
		return;
	}
	ServerMediaSubsession* subsession = NULL;
	...
	if (strcmp(cmdName, "TEARDOWN") == 0) {
		...
		Debug(ckite_log_message, "handleCmd_withinSession  PAUSE start\n");
...
::handleCmd_PLAY(ServerMediaSubsession* subsession, char const* cseq,
	...
	if( strcmp(fPath, "store") == 0 )
	{
		...
		if(parseSpecifiedStringParam(fullRequestStr,magicType,(char *)"magic:",6))
		{
			...
			Debug(ckite_log_message, "magicType = %s\n", magicType);
		...
		Debug(ckite_log_message, "videofp = %x\n", videofp);
		if (videofp == NULL)
		{
			Debug(ckite_log_message, "open video file is failed.\n");
			return ;
		}
	}
	...
	// Now, start streaming:
	Debug(ckite_log_message, "fNumStreamStates = %d\n", fNumStreamStates);
	for (i = 0; i < fNumStreamStates; ++i) {
		if (subsession == NULL /* means: aggregated operation */
			|| subsession == fStreamStates[i].subsession) {
...
::handleCmd_PAUSE(ServerMediaSubsession* subsession, char const* cseq) {
	...
	Debug(ckite_log_message, "handleCmd_PAUSE\n");
	fprintf(stderr, "handleCmd_PAUSE\n");
	for (unsigned i = 0; i < fNumStreamStates; ++i) {
		if (subsession == NULL /* means: aggregated operation */
			|| subsession == fStreamStates[i].subsession) {
...
::handleCmd_GET_PARAMETER_SubDirList(ServerMediaSubsession* subsession, char const* cseq,
	...
	Debug(ckite_log_message, "list_param = %s\n", list_param);
	...
	Debug(ckite_log_message, "count = %d, index = %d\n", count, index);
	if(dirList == NULL)
	{
		dirList = new (char *[getCount]);
		//Debug(ckite_log_message, "dirList address = %p\n", dirList);
	}
	...
	Debug(ckite_log_message, "content = %s\n", content);
	...
	Debug(ckite_log_message, "dirList free = %p\n", dirList);
...
::handleCmd_GET_PARAMETER_SubFileList(ServerMediaSubsession* subsession, char const* cseq,
	...
	Debug(ckite_log_message, "handle by list file.\n");
	...
	while( pos[i] != ';')
	{
		count_str[i] = pos[i];
		i++;
		Debug(ckite_log_message, "i = %d, pos[i] = %c\n", i, pos[i]);
	}
	i = 0;
	getCount = count = atoi(count_str);
	...
	index = atoi(index_str);
	Debug(ckite_log_message, "list file count = %d, index = %d\n", count, index);
...
::do_heartbeat(bool with_sdp, bool with_challenge_resp, char* challenge_resp)
{
	...
	if (ret > 0)
	{
		...
		fprintf(stderr, "send heartbeat msg is sucess.\n");
		char updateState[60] = {0};
		LPhikProfile proInfo = getProfileParamenterInfo();
		snprintf(updateState, sizeof(updateState), "..//..//exec//challenge%d.ini", proInfo->processID);
		FILE * fp = fopen(updateState, "wb+");
