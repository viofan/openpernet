...
{
	int i;
	fprintf(stderr, "fd_set has %d elems: ", set->fd_count);
	for (i=0; i<set->fd_count; i++)
	{
		fprintf(stderr, "\t[%08x]", set->fd_array[i]);
	}
	fprintf(stderr, "\n");
}
...
void BasicTaskScheduler::SingleStep(unsigned maxDelayTime) {
 ...
 // dump_fdset(&readSet);
  ...
  while ((handler = iter.next()) != NULL) {
	...
	{
		fprintf(stderr, "*============================Read event on socket %08x\n", sock);
		fprintf(stderr, "resultConditionSet=%d, handler->conditionSet=%d, handler->handlerProc=%p\n",
		resultConditionSet, handler->conditionSet, handler->handlerProc);
