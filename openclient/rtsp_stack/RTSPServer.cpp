...
void RTSPServer::RTSPClientSession::incomingRequestHandler(void* instance, int /*mask*/) {
	fprintf(stderr, "RTSPClientSession::incomingRequestHandler.\n");
...
void RTSPServer::RTSPClientSession::incomingRequestHandler1() {
	...
	memcpy(tempBuf, &fRequestBuffer[fRequestBytesAlreadySeen], bytesRead);
	Debug(ckite_log_message, "%s\n", &fRequestBuffer[fRequestBytesAlreadySeen]);
	fprintf(stderr, "%s\n", tempBuf);
...
#else
		Debug(ckite_log_message, "urlSuffix = %s\n", urlSuffix);
...
::authenticationOK(char const* cmdName, char const* cseq,
	...
	if (!fOurServer.specialClientAccessCheck(fClientSocket, IsHttp?fHttpGetClientAddr:fClientAddr, urlSuffix)) {
		snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
