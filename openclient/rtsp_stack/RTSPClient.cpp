...
unsigned RTSPClient::sendRequest(RequestRecord* request) {
  ...
  do {
	...
	transportFmt = "Transport: RTP/AVP/TCP%s%s%s=%d-%d\r\n"; // wayde
      }
...
void RTSPClient::handleIncomingRequest() {
  ...
  if (!parseRTSPRequestString((char *)fResponseBuffer, fResponseBytesAlreadySeen,
...
static unsigned char tempStreamBuffer[20*1000];
unsigned char *out_buffer = NULL;//new unsigned char [FRAME_WIDTH*FRAME_HEIGHT*4];
static void *dec_handle;
static unsigned dataTotal=0;
static 	int XDIM = 0;
static 	int YDIM = 0;
#define MIN_USEFUL_BYTES 5

static int
decoderDecodeEntry(unsigned char *istream,
...
//	xvid_dec_frame.output.plane[1]  = ostream + FRAME_WIDTH*FRAME_HEIGHT;
//	xvid_dec_frame.output.plane[2]  = ostream + FRAME_WIDTH*FRAME_HEIGHT*5/4;
	xvid_dec_frame.output.stride[0] = XDIM;//FRAME_WIDTH;
//	xvid_dec_frame.output.stride[1] = FRAME_WIDTH/2;
//	xvid_dec_frame.output.stride[2] = FRAME_WIDTH/2;
...
//	xvid_gbl_init.debug = debug_level;
	 ...
	 * needed
	*/
	xvid_dec_create.height = 0;//FRAME_WIDTH;
	xvid_dec_create.width = 0;//FRAME_HEIGHT;
...
Boolean FrameStreamConnectDecoder(void *client, unsigned char *frame, unsigned useful_bytes)
{
	...
	{
		int used_bytes = 0;
		//fprintf(stderr, "dec_handle = %x\n", dec_handle);
		used_bytes = decoderDecodeEntry(framePtr, out_buffer, useful_bytes, &xvid_dec_stats);
		if(xvid_dec_stats.type == XVID_TYPE_VOL) {
			...
			if(XDIM*YDIM < xvid_dec_stats.data.vol.width*xvid_dec_stats.data.vol.height) {
				...
				/* Allocate the new buffer */
					out_buffer = new unsigned char[XDIM*YDIM*4];
					if (out_buffer == NULL)
					{
 ...
 ****************************************************************************/
	...
	do {
		used_bytes = decoderDecodeEntry(NULL, out_buffer, -1, &xvid_dec_stats);
...
void RTSPClient::handleResponseBytes(int newBytesRead) {
  ...
  do {
    ...
    strncpy(headerDataCopy, (char *)fResponseBuffer, fResponseBytesAlreadySeen);
